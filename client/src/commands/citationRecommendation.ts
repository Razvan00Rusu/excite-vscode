...
export async function recommendCitation(context: vscode.ExtensionContext): Promise<CommandResult> {
	...
	const localRecommendation = await recommendReferenceByLocalContext(selectedText);
	// const globalRecommendation = await recommendReferenceByGlobalContext(
	//   removeExcessWhitespace
	// );
	const globalRecommendation: any[] = [];
        ...
        ${
					localRecommendation?.inference[0].result
						...
						.map((el, idx) => {
							return `
            <div>
              <p class="recommendationTitle" onClick="submitSelection('${el.title}')">${
								idx + 1
							}. ${el.title}</p>
              <small>
              By: ${el.author}
	...
	panel.webview.onDidReceiveMessage(async (message) => {
		...
		if (message.command === 'selected') {
			...
			const lastCharacter = editor.document.getText(
				new vscode.Range(primarySelection.end.translate(0, -1), primarySelection.end)
			);
			...
			if (!findDetails || !findDetails[0].doi) {
				vscode.window.showErrorMessage('Could not retrieve data for this source.');
				return;
			}
      ...
      runCommand('pandoc', [
			...
			if (!insertCiteKey) {
				vscode.window.showErrorMessage('unable to insert citekey into active document');
			}
		}
	});
